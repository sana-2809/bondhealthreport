...
\justifying
The Automated Diagram Digitization platform utilizes PostgreSQL with JSONB support, providing reliable data persistence, ACID compliance, and flexible schema for storing complex flowchart data.The schema uses UUID primary keys, timestamp tracking, and JSONB columns for flow data storage.~\cite{postgresql}
...
The following table provides a concise summary of the \texttt{flowcharts} table structure as implemented in PostgreSQL. \textit{Refer Table~\ref{tab:database-schema}: Flowcharts Table Schema}.
...
\subsection{Database Indexes for Performance Optimization}

\justifying
Database indexing is essential for maintaining high performance and responsiveness as the storage volume of flowcharts grows. The following indexes were implemented to optimize common query patterns, ensuring that diagram retrieval, sorting, and JSON-based searches remain efficient. By creating these targeted indexes, the system avoids full table scans during data retrieval, significantly reducing the latency of API calls. This is particularly crucial for the JSONB columns, where specialized GIN indexes enable the database to efficiently traverse nested flowchart data.
...
\justifying
The editor module provides a high-performance interactive workspace where users can refine and personalize their digitized diagrams. Built on React Flow, it handles complex state management and real-time canvas updates, ensuring a seamless experience when dragging, resizing, or styling flowchart components. Components include:
...
\justifying
The storage module is responsible for the persistent management of flowchart data, ensuring that user work is saved reliably and can be retrieved across sessions. It leverages SQLAlchemy to provide an asynchronous abstraction over the PostgreSQL database, utilizing the JSONB data type to store complex diagram states with high performance and flexibility. Components include:
...
\justifying
The theme module delivers a customized visual experience by managing the application's appearance settings. It coordinates the synchronization of dark and light mode preferences using React Context and CSS variables, ensuring that theme changes are applied instantly throughout the entire component hierarchy without requiring a full page reload. Components include:
...
\justifying
The column-based auto layout algorithm is responsible for organizing nodes in a flowchart so that they appear visually aligned and structured. When diagrams are generated automatically by the AI system or manually edited by users, node positions may not always be perfectly aligned. The auto layout algorithm corrects this by grouping nodes into vertical columns based on their horizontal positions. The algorithm begins by determining the maximum width required for each node type. Node types such as start nodes, process nodes, decision nodes, and input/output nodes may have different sizes. By determining the largest width for each type, the algorithm ensures visual consistency across the diagram.

After establishing the node widths, the algorithm calculates the horizontal center position of each node using its current position and width. These center positions are then used to sort the nodes along the horizontal axis. Sorting enables the algorithm to identify nodes that are positioned near one another. Once sorted, the algorithm performs a clustering process in which nodes whose center positions fall within a defined tolerance range are grouped together into a single column. Each column maintains an average center value calculated from all nodes assigned to that group. In the final stage, the algorithm repositions each node so that its center aligns with the calculated average center of its column. This ensures that nodes belonging to the same logical stage of a flowchart appear vertically aligned while maintaining their original vertical ordering. As a result, the flowchart becomes visually balanced and easier to interpret.
...
\justifying
Providing a seamless and responsive user interface is essential for professional diagramming tools. Our performance optimization strategy focuses on reducing latency and minimizing resource consumption across all system layers, from efficient binary transmission on the backend to optimized rendering cycles in the interactive editor.
